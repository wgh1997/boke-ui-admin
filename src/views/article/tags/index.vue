...
import { postArticleTags, getArticleTags, putArticleTags, deleteArticleTags } from "@/api/modules/article";
...
// 表格 hooks
const { tableData, pageable, getTableList, handleSizeChange, handleCurrentChange } = useTable(getArticleTags, initParam);
...
const deleteAccount = async (params: Article.Classify) => {
	await useHandleData(deleteArticleTags, { id: params.id }, `删除【${params.name}】`);
	getTableList();
};
...
const openDrawer = (title: string, rowData: Partial<Article.ArticleList> = {}) => {
	let params = {
		...
		rowData: { ...rowData },
		isView: title === "查看",
		apiUrl: title === "新增" ? postArticleTags : title === "编辑" ? putArticleTags : "",
		getTableList
	};
